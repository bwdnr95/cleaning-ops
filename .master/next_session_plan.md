...
작성일: 2026-05-18  
목적: R7 다중 상품 주문 작업 이후 상태를 빠르게 파악하고, 다음 cleanup/운영 기능을 안전하게 이어가기 위한 handoff.
...
- 검증 완료:
  - backend `python -m pytest -q` -> `104 passed`
  - frontend `npm run typecheck`
  - frontend `npm run lint`
  - frontend `npm run build` -> Vite chunk size warning만 있음
  - frontend `npm run e2e` -> `22 passed`
...
**R7.5 Deprecated Order Customer Columns Cleanup**

목표:

R7에서 호환을 위해 남겨둔 `orders.customer_token`, `orders.customer_name`, `orders.customer_phone`, `orders.customer_address`, `orders.source_channel`, `orders.customer_visible_payment`를 제거할 준비를 한다.

권장 순서:

1. `rg "customer_token|customer_name|customer_phone|customer_address|source_channel|customer_visible_payment" backend/app frontend/src backend/tests`로 legacy `Order` 컬럼 의존도를 다시 확인한다.
2. 아직 `Order`에서 customer 정보를 읽는 dashboard/calendar/message/partner DTO 경로를 group join 또는 explicit group 인자로 전환한다.
3. import/외부 연동 스크립트가 있다면 group 생성 API를 사용하도록 바꾼다.
4. Alembic cleanup migration을 별도 PR로 만든다.
5. backend 테스트와 E2E를 전체 실행한다.
...
- R7 동안 legacy `orders.customer_*` 컬럼은 호환 미러링 용도로 남아 있다. R7.5에서 제거해야 한다.
...
R7.5 Deprecated Order Customer Columns Cleanup을 진행해줘.

먼저 legacy orders.customer_* 컬럼 의존도를 rg로 확인하고,
서비스/DTO/화면이 OrderGroup을 source of truth로 읽도록 전환한 뒤
cleanup migration 계획을 세워줘. 권한/DTO/timeline 규칙은 AGENTS.md 기준으로 유지해줘.
```
