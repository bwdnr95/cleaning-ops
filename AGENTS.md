...
주문 상태는 `backend/app/domain/constants.py`의 `OrderStatus` enum(현재 14개 값)을 단일 기준으로 한다. (브리프의 13개에서 `고객확인필요`가 추가됨.)
...
- `고객전달필요`
- `고객전달완료`
- `고객확인필요` — 컴플레인/이슈로 최종결제(미수금)가 막힌 건을 따로 관리하는 보류 상태. 최종결제완료 직전 단계이며 자동 전이 경로는 없고 운영자가 명시적으로 지정한다.
- `서비스완료`
- `취소`
