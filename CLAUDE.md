...
Partner upload → `is_customer_visible=false`, order status flips to `사진검수대기`. Admin approval → `is_customer_visible=true`, status flips to `고객전달필요`. Customer DTO returns only `is_customer_visible=true` photos. File type is verified by **byte signature**, not just MIME (see `services/photos.py`).
