...
def test_partner_photo_upload_locks_order_before_status_validation() -> None:
    order = Order(
        id="order-1",
        group_id="group-1",
        status=OrderStatus.CUSTOMER_DELIVERY_NEEDED.value,
    ...
    with pytest.raises(ValueError, match="invalid_status_for_upload"):
        service.upload_for_partner(
            payload=cast(Any, type("Payload", (), {"order_id": "order-1"})()),
            user_id="partner-user-1",
            partner_id="partner-1",
        )
