...
EXPECTED_SOLAPI_KEYS = {
    ...
    MessageType.CUSTOMER_PHOTO_READY: {"#{고객명}", "#{서비스명}"},
    MessageType.CUSTOMER_BALANCE_DUE: {"#{고객명}", "#{서비스명}", "#{잔금}"},
...
def test_kakao_render_fills_every_variable_without_blank():
    ...
    context = {
        ...
        "discount_amount": "0원", "customer_link": "http://x/c/tok", "partner_name": "청소왕",
    ...
    assert schedule["#{고객링크}"] == "http://x/c/tok"
