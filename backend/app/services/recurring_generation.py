...
from app.domain.constants import OrderStatus, RecurringBillingMode, VatType
        ...
        *,
        ...
        with self.db.begin_nested():
            for _seq, due in iter_due_dates(recurring_schedule_spec(contract), until=window.last):
                if due < window.first or due > window.last or due in existing:
                    continue
    ...
    def _contract_line_payload(self, contract: RecurringContract, due: date) -> OrderLineCreate:
        ...
        return OrderLineCreate(
            ...
            partner_payment_amount=(
                to_float(contract.partner_payment_amount) if is_partner_per_visit else None
            ),
        )
