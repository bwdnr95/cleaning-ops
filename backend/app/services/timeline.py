...
class TimelineService:
    ...
    def latest_current_partner_confirmation(
        self, *, order_id: str, partner_id: str | None
    ) -> datetime | None:
        if not partner_id:
            return None
        confirmed = self.repo.latest_event(
            order_id=order_id, event_type=TimelineEventType.PARTNER_CONFIRMED
        )
        if confirmed is None or (confirmed.event_metadata or {}).get("partner_id") != partner_id:
            return None
        required = self.repo.latest_event(
            order_id=order_id, event_type=TimelineEventType.PARTNER_CONFIRMATION_REQUIRED
        )
        assigned = self.repo.latest_event(
            order_id=order_id, event_type=TimelineEventType.PARTNER_ASSIGNED
        )
        epoch_start = max(
        ...
        return None if epoch_start is not None and confirmed_at <= epoch_start else confirmed_at
    ...
    def latest_partner_work_epoch(
        ...
        *,
        ...
        as_requested = self.repo.latest_event(
            order_id=order_id, event_type=TimelineEventType.AS_REQUESTED
        )
        if as_requested is not None and (as_requested.event_metadata or {}).get("source") == "customer":
            as_requested = None
        assigned = self.repo.latest_event(
            order_id=order_id, event_type=TimelineEventType.PARTNER_ASSIGNED
        )
    ...
    def latest_accepted_as_created_at(
        ...
        *,
        ...
        return self.repo.latest_accepted_as_created_at(
            order_id=order_id,
            active_as_request_id=active_as_request_id,
        )
